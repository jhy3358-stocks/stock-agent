...
매일 하루 3회(KST 약 08:00/17:00/21:30) 국내/미국 주요 종목·지수의 기술적 분석 리포트를 자동 생성해 카카오톡 "나에게 보내기"로 발송합니다.
...
- `.github/workflows/daily_report.yml` — 매일 KST 약 08:07/17:07/21:37 자동 실행 (GitHub Actions, 정각 지연 회피를 위해 소폭 오프셋)
...
4. **Actions** 탭에서 `Daily Stock Report` 워크플로우 확인
   - 매일 KST 약 08:07/17:07/21:37(UTC 23:07/08:07/12:37) 자동 실행 — 데이터 수집 →
     `docs/index.html` 갱신 후 자동 커밋/push → 카카오톡 요약 메시지 발송
   - GitHub Actions 스케줄은 실행 시각을 엄격히 보장하지 않으며(특히 정각에 부하가 몰릴 때),
     수 분~수 시간 지연될 수 있습니다. 위 시각은 목표치이며 정확히 그 시각에 온다는 보장은 아닙니다
